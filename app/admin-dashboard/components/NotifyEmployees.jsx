...
const NotifyAllEmployees = ({ incidentId, incidentData }) => {
  ...
  const handleSendNotification = async () => {
    ...
    try {
      ...
      // 🔁 CHANGED: write to notification/{company}/notify
      const ref = await addDoc(
        collection(db, 'notification', userCompany, 'notify'),
        notificationData
      );

      console.log('Notification created at:', `notification/${userCompany}/notify/${ref.id}`);
  ...
  return (
    <div className="bg-white rounded-2xl shadow-md p-6 max-w-sm mx-auto">
      ...
      >
        <span className="text-base font-medium">{isSending ? 'Sending...' : 'Send to All Employees'}</span>
        {isSending ? <Loader2 className="w-6 h-6 animate-spin" /> : <ChevronRight className="w-6 h-6 stroke-[3] group-hover:translate-x-1 transition-transform" />}
      </button>
    </div>
  );
};
